...
class FreeplayState extends MusicBeatState
{
	...
	function changeDiff(change:Int = 0)
	{
		...
		if (curDifficulty < 0 && curSelected != 3)
			curDifficulty = CoolUtil.difficultyStuff.length-2;
		if (curDifficulty >= CoolUtil.difficultyStuff.length - 1 && curSelected != 3)
			curDifficulty = 0;
	...
	function changeSelection(change:Int = 0)
	{
		...
		if (curSelected >= songs.length)
			curSelected = 0;
		if (curSelected == 3 && curDifficulty != 4)
		{
			lastDif = curDifficulty;
			curDifficulty = 4;
			changeDiff(0);
		}
		else if (curSelected != 3 && curDifficulty == 4)
		{
