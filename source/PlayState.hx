...
class PlayState extends MusicBeatState
{
	...
	var canAnimateDad:Bool = true;
	var cock:FlxSprite = new FlxSprite().loadGraphic(Paths.image('dualrasis_glow', 'shared'));
	var shutOff:FlxSprite;
	var sexOff:FlxSprite;
	var fuckOff:FlxSprite;
	var andBallTorture:FlxSprite = new FlxSprite().loadGraphic(Paths.image('dualtama_glow', 'shared'));
	var bgBlack:FlxSprite = new FlxSprite().makeGraphic(450, 2000, FlxColor.BLACK);
	var bgBlack2:FlxSprite = new FlxSprite().makeGraphic(450, 2000, FlxColor.BLACK);
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['S', 0.99],
		['Perfect!!', 1]
	]; // copy paste this array ^

	public var scrollVariation:Int = 0;
	#if (haxe >= "4.0.0")
	...
	public var defaultCamZoom:Float = 0.3;
	...
	{
		cock.screenCenter();
		shutOff = new FlxSprite();
		shutOff.frames = Paths.getSparrowAtlas('Sex', 'shared');
		shutOff.animation.addByPrefix('Sex', 'sex', 60, false);
		// shutOff.setGraphicSize(Std.int(shutOff.width * 2));
		shutOff.scrollFactor.set();
		shutOff.updateHitbox();
		shutOff.screenCenter();
		sexOff = new FlxSprite();
		sexOff.frames = Paths.getSparrowAtlas('Sex2', 'shared');
		sexOff.animation.addByPrefix('Sex2', 'sex 2, the squeakquel', 60, false);
		// sexOff.setGraphicSize(Std.int(sexOff.width * 2));
		sexOff.scrollFactor.set();
		sexOff.updateHitbox();
		sexOff.screenCenter();
		fuckOff = new FlxSprite();
		fuckOff.frames = Paths.getSparrowAtlas('Sex3', 'shared');
		fuckOff.animation.addByPrefix('Sex3', 'sex 3, the enemy returns', 60, false);
		// fuckOff.setGraphicSize(Std.int(fuckOff.width * 2));
		fuckOff.scrollFactor.set();
		fuckOff.updateHitbox();
		fuckOff.screenCenter();

		andBallTorture.screenCenter();
		cock.x += 75;
		andBallTorture.x += 75;
		cock.y -= 75;
		andBallTorture.y -= 75;
		...
		if (ClientPrefs.bgShadow)
		{
			...
			bgBlack.alpha = ClientPrefs.bgOppacity / 100;
			if (!ClientPrefs.middleScroll)
			{
				bgBlack2.screenCenter();
				bgBlack2.x -= 325;
				bgBlack2.cameras = [camHUD];
				bgBlack2.alpha = ClientPrefs.bgOppacity / 100;
				add(bgBlack2);
			}
			else
				bgBlack.x -= 318;
		...
		{
			case 'stage': // Week 1
				var bg:BGSprite = new BGSprite('stageBacks/' + songName, -600, -200, 1, 1);
				bg.screenCenter();
				add(bg);
				bg2 = new BGSprite('BG_back_finale', -600, -200, 1, 1);
				bg2.screenCenter();
				bg2.alpha = 0;
				add(bg2);
				add(andBallTorture);
				add(cock);
				stageFront = new BGSprite('BG_floor_symmetrical', -600, -200, 0.9, 0.9);
		...
		#end
		cock.visible = false;
		andBallTorture.visible = false;
		goUpGoDownOnDeezDick();
		super.create();
		add(shutOff);
		add(sexOff);
		add(fuckOff);
	...
	function sPINNNN()
	{
		FlxTween.tween(andBallTorture, {angle: 180}, 8, {
			ease: FlxEase.linear,
			onComplete: function(tween:FlxTween)
			{
				FlxTween.tween(andBallTorture, {angle: 360}, 8, {
					ease: FlxEase.linear,
					onComplete: function(tween:FlxTween)
					{
						andBallTorture.angle = 0;
						sPINNNN();
					}
				});
			}
	...
	function goUpGoDownOnDeezDick()
	{
		FlxTween.tween(cock, {y: pos + 30}, 2, {
		...
		FlxTween.tween(andBallTorture, {y: pos + 30}, 2, {
			ease: FlxEase.sineInOut,
			onComplete: function(tween:FlxTween)
			{
				FlxTween.tween(andBallTorture, {y: pos - 30}, 2, {
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				{
					if (boyfriend.animation.curAnim != null && !boyfriend.animation.curAnim.name.startsWith('sing') && canAnimate == true)
					{
						boyfriend.dance();
					}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		/*if (SONG.needsVoices && storyDifficultyText != 'Maximum')
			vocals = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song + ''));*/
		if (storyDifficultyText == 'Maximum')
			vocals = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song + '-max'));
		else
			vocals = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song + ''));
					...
					if (floorSus > 0)
					{
						for (susNote in 0...floorSus + 1)
						{
							...
							var sustainNote:Note = new Note(daStrumTime
								...
								+ (Conductor.stepCrochet / FlxMath.roundDecimal(ClientPrefs.scrollSpeed, 2)),
	...
	var roundedSpeed:Float = FlxMath.roundDecimal(ClientPrefs.scrollSpeed, 2) /* FlxMath.roundDecimal(ClientPrefs.scrollSpeed, 2)*/;
	...
	override public function update(elapsed:Float)
	{
		...
		{
			var achieve:Int = checkForAchievement([0,1,2,3]);
			if (achieve > -1)
			{
		...
		if (deez == curStep - 1 && dad.animation.curAnim.finished)
		{
			canAnimateDad = true;
			dad.dance();
		}
		if (nutz == curStep - 1 && boyfriend.animation.curAnim.finished)
		{
			canAnimate = true;
			boyfriend.dance();
		}
		...
		{
			scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Rating: ' + ratingString + ' ('
				+ FlxMath.roundDecimal(ratingPercent * 100, 2) + '%)';
		} // copy paste this else statement
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				// daNote.y = (strumLine.y - (songTime - daNote.strumTime) * (0.45 * PlayState.ClientPrefs.scrollSpeed + 1));
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			case 'Combro told me to change the name of this event or else we will get canceled':
				...
				else if (Std.parseFloat(value1) == 1)
				{
					dad.visible = false;
					cock.visible = true;
					andBallTorture.visible = true;
				}
				else if (Std.parseFloat(value1) == 2)
				{
					sPINNNN();
					dad.visible = false;
					cock.visible = true;
			...
			case 'Shut Off':
				...
				shutOff.alpha = 1;
				sexOff.alpha = 1;
				shutOff.animation.play('Sex');
				sexOff.animation.play('Sex2');
			case 'Shut Off2':
				fuckOff.alpha = 1;
				stageFront.alpha = 1;
				bg2.alpha = 1;
	...
	override function beatHit()
	{
		...
		if (curBeat % 2 == 0)
		{
			if (boyfriend.animation.curAnim.name != null && !boyfriend.animation.curAnim.name.startsWith("sing") && canAnimate == true)
			{
				boyfriend.dance();
			}
			if (dad.animation.curAnim.name != null
				&& !dad.animation.curAnim.name.startsWith("sing")
				&& !dad.stunned
				&& canAnimateDad == true)
			{
		...
		for (i in 0...arrayIDs.length)
		{
			if (!Achievements.achievementsUnlocked[arrayIDs[i]][1])
			{
				switch (arrayIDs[i])
				{
					case 0:
						if (isStoryMode &&
							CoolUtil.difficultyString() == 'MAXIMUM' &&
							storyPlaylist.length <= 0
							&& WeekData.getWeekFileName() == ('week1')
							&& !changedDifficulty
							&& !usedPractice)
						{
					...
					case 1:
						if (isStoryMode && CoolUtil.difficultyString() == 'MAXIMUM'
							&& storyPlaylist.length <= 0
							&& WeekData.getWeekFileName() == ('week1')
							&& !changedDifficulty
							&& !usedPractice)
						{
							if(campaignMisses + songMisses < 69)
							{
								Achievements.unlockAchievement(arrayIDs[i]);
								return arrayIDs[i];
							}
					...
					case 2:
						if (CoolUtil.difficultyString() == 'NOVICE'
							&& storyPlaylist.length <= 0
							&& changedDifficulty)
						{
							Achievements.unlockAchievement(arrayIDs[i]);
					...
					case 3:
						if (storyPlaylist.length <= 0
							&& WeekData.getWeekFileName() == ('week2')
							&& !changedDifficulty
							&& !usedPractice)
						{
							Achievements.unlockAchievement(arrayIDs[i]);
