...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			if(Achievements.achievementsUnlocked[0][1] == false)
			{
				if (weekIsLocked(i))
				{
	...
	function selectWeek()
	{
		if ((!weekIsLocked(curWeek) || Achievements.achievementsUnlocked[0][1] == true) || Achievements.achievementsUnlocked[0][1] != false)
		{
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		if (curWeek >= WeekData.weeksList.length)
			curWeek = 0;
		// exper, only here because the difficulties aren't implemented proper yet
		// KMD here, relax i'll do it UwU
		...
		if (curWeek == 1 && curDifficulty != 4)
		{
			lastDif = curDifficulty;
			curDifficulty = 4;
			changeDifficulty(0);
		}
		else if (curWeek != 1 && curDifficulty == 4)
		{
			curDifficulty = lastDif;
			changeDifficulty(0);
		}
		if (weekIsLocked(curWeek) && Achievements.achievementsUnlocked[0][1] == false)
			changeWeek(-1);
