...
class NotesSubstate extends MusicBeatSubstate
{
	...
	public function new() {
		...
		for (i in 0...ClientPrefs.arrowHSV.length) {
			...
			var noteSkin = ClientPrefs.noteSkin;
			/*if (noteSkin == 'Bar')
				note.frames = Paths.getSparrowAtlas('barSkin');
			else if (noteSkin == 'Arrow')*/
...
{
	public var SMult:Float = 1.0; // copy paste this float
	private static var curSelected:Int = 0;
	...
	override function update(elapsed:Float)
	{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					case 'BG Shadow':
						ClientPrefs.bgShadow = !ClientPrefs.bgShadow;
					case 'FPS Counter':
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				switch(options[curSelected]) {
					...
					case 'Scroll Speed':
						SMult = 1.0;
						ClientPrefs.scrollSpeed += add * 0.1;
						if(ClientPrefs.scrollSpeed < 0) ClientPrefs.scrollSpeed = 0;
						else if(ClientPrefs.scrollSpeed > 15) ClientPrefs.scrollSpeed = 15;
					case 'BG Shadow Oppacity':
						ClientPrefs.bgOppacity += add * 10;
						if(ClientPrefs.bgOppacity < 0) ClientPrefs.bgOppacity = 0;
						else if(ClientPrefs.bgOppacity > 100) ClientPrefs.bgOppacity = 100;
				} // copy paste the case scroll speed thing
				reloadValues();
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Scroll Speed':
				daText = "Changes and overwrites the scroll speed of the song in case it is above or below 1.";
			case 'BG Shadow':
				daText = "Adds a black box behind the notes to make reading them easier";
			case 'BG Shadow Oppacity':
				daText = "Changes the oppacity of that black box behind the notes";
			case 'FPS Counter':
		...
		} // copy paste the third case. it's scroll speed, whaddayaexpect
		...
		for (i in 0...grpTexts.members.length) {
			...
			if(text != null) {
				...
				switch(options[textNumber[i]]) {
					...
					case 'BG Shadow Oppacity':
						daText = ClientPrefs.bgOppacity + '';
				} // copy paste this  scroll speed too
